...
def add_info(report, ui):
    ...
    ddproc = Popen(['dpkg-divert', '--truename', '/usr/bin/thunderbird'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    truename = ddproc.communicate()
    if ddproc.returncode == 0 and truename[0].strip() != '/usr/bin/thunderbird':
        ddproc = Popen(['dpkg-divert', '--listpackage', '/usr/bin/thunderbird'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    appini = AppINIParser('/usr/lib/thunderbird-7.0.1')
    ...
    syspref = PrefFile("/etc/thunderbird/syspref.js")
    if syspref.available == False:
        report["EtcThunderbirdSyspref"] = "Cannot parse /etc/thunderbird/syspref.js - %s" % syspref.parse_error
    for pref in syspref:
        report["EtcThunderbirdSyspref"] += "%s - %s\n" % (pref, syspref[pref])
    ...
    if 'thunderbird' == 'thunderbird-trunk':
        ...
        channelpref = PrefFile("/usr/lib/thunderbird-7.0.1/defaults/pref/channel-prefs.js", whitelist = [ r'app\.update\.channel' ])
