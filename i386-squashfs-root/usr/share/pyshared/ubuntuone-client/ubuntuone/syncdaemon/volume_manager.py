...
class Share(Volume):
    ...
    def __init__(self, volume_id=None, node_id=None, path=None, name=None,
                 ...
                 access_level='View', free_bytes=None, generation=None,
                 subscribed=False):
        """Create the share."""
    ...
    def active(self):
        ...
        return self.accepted and self.subscribed
...
class Root(Volume):
    ...
    def from_volume(cls, volume):
        ...
        return cls(node_id=str(volume.node_id), free_bytes=volume.free_bytes, generation=volume.generation)

    def __repr__(self):
        return "<Root node_id %r, generation %r>" % (self.node_id, self.generation)
    ...
    def _volumes_rescan_cb(self, volumes):
        ...
        for new_volume in volumes:
            ...
            if isinstance(new_volume, RootVolume):
                ...
                # don't check the fsm node, the root fsm object is created in init_root
    ...
    def _guess_metadata_version(self):
        ...
        if path_exists(self._shares_md_dir) \
           and path_exists(self._shared_md_dir):
            ...
            # md_version >= 1
            old_root_dir = os.path.abspath(os.path.join(self._root_dir, 'My Files'))
            ...
            if path_exists(old_share_dir) and path_exists(old_root_dir) \
               and not is_link(old_share_dir):
                ...
                if is_link(self._shares_dir_link) \
                   and normpath(target) == os.path.abspath(self._shares_dir_link):
