...
def assert_syncdaemon_path(path, method_name=None):
    ...
    * do not contain the LONG_PATH_PREFIX
    ...
    messages = {
        ...
        'unicode_chars': '%r should not contain any character from LINUX_ILLEGAL_CHARS_MAP.',
    }
    messages = _add_method_info(messages, method_name)
...
def syncdamonpath(path_indexes=None):
    ...
    The paths given by the indexes from 'path_indexes' are validated to be
    'syncdaemon' paths,  and are also transformed to be valid 'syncdaemon' path,
    using the get_syncdaemon_valid_path transformer. If 'path_indexes' is None,
    only the first argument is checked.
...
def rename(path_from, path_to):
    ...
    return _rename(path_from, path_to)
...
def _rename(path_from, path_to):
    ...
    # Also, to ensure the same behaviors as on linux, use the MoveFileExW function
    # from win32 which will allow to replace the destination path if exists
    # and the user has the proper rights. For further information, see:
...
def make_link(target, destination):
    ...
    try:
        shortcut = shell_script.CreateShortCut(destination)
...
def read_link(path):
    ...
    result = get_syncdaemon_valid_path(LONG_PATH_PREFIX + shortcut.Targetpath)
    return result
...
def listdir(directory):
    ...
    return map(_unicode_to_bytes, os.listdir(directory))
...
def walk(path, topdown=True):
    ...
    for dirpath, dirnames, filenames in os.walk(path, topdown):
        ...
        dirnames = map(_unicode_to_bytes, dirnames)
        filenames = map(_unicode_to_bytes, filenames)
...
def move_to_trash(path):
    ...
    # the full task you asked of it and you might proceed under incorrect
    # assumptions.
